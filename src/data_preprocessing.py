...
def load_data_with_split(adata, pdata, spot_patches, config, split):
    gene_data = torch.tensor(adata.layers['norm'].todense()).float()
    gene_data = filter_genes_by_variance(gene_data, target_num_genes=config['hyperparameters']['target_num_genes'])
    ...
    if split == 'train':
        if config['hyperparameters']['use_validation']:
            ...
            return (
                (gene_data[train_indices], protein_data[train_indices], spot_patches[train_indices]),
                (gene_data[val_indices], protein_data[val_indices], spot_patches[val_indices])
            )
        else:
            return (gene_data, protein_data, spot_patches), None
